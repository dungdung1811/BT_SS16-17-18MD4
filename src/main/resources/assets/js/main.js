$(document).ready(function () {
	...
	$('.home__slider').on('initialized.owl.carousel changed.owl.carousel', function(e) {
		...
		$('.home__counter').html('0' + (carousel.relative(carousel.current()) + 1) + ' ' + '/' + ' 0' + carousel.items().length);
	}).owlCarousel({
		mouseDrag: false,
		touchDrag: true,
		items: 1,
	...
	$('.product-head__slider').on('initialized.owl.carousel changed.owl.carousel', function(e) {
		if (!e.namespace) return;
		var carousel = e.relatedTarget;
		$('.product-head__counter').html('0' + (carousel.relative(carousel.current()) + 1) + ' ' + '/' + ' 0' + carousel.items().length);
	}).owlCarousel({
		mouseDrag: true,
		touchDrag: true,
		items: 1,
